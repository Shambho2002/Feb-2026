...
public class TernarySearch3{
	public static int search(int[] arr, int Skey){
		
		// Ternary Search Algorithm
		
		int left = 0; // start fist index
		int right = arr.length - 1; // end last index
		
		boolean asc = arr[left] < arr[right];
		
		while(left <= right){
			
			int mid1 = left + (right - left) / 3; // for found the mid1
			int mid2 = right - (right - left) / 3; // for found the mid2
	...
	public static void main(String[] args){
		...
		System.out.println("Output: "+search(arr, Skey));
	}
}